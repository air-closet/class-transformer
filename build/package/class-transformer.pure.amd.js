            ...
            if (Array.isArray(value) || value instanceof Set) {
                ...
                if (!(value instanceof targetType)) {
                    ...
                    var _loop_2 = function (key) {
                        var valueKey = key, newValueKey = key, propertyName = key;
                        if (!this_2.options.ignoreDecorators && targetType) {
                            if (this_2.transformationType === TransformationType.PLAIN_TO_CLASS) {
                                var exposeMetadata = storage_1.defaultMetadataStorage.findExposeMetadataByCustomName(targetType, key);
                                if (exposeMetadata) {
                                    propertyName = exposeMetadata.propertyName;
                                    newValueKey = exposeMetadata.propertyName;
                            ...
                            else if (this_2.transformationType === TransformationType.CLASS_TO_PLAIN || this_2.transformationType === TransformationType.CLASS_TO_CLASS) {
                                var exposeMetadata = storage_1.defaultMetadataStorage.findExposeMetadata(targetType, key);
                                if (exposeMetadata && exposeMetadata.options && exposeMetadata.options.name) {
                                    newValueKey = exposeMetadata.options.name;
                                }
                        ...
                        if (targetType && isMap) {
                            ...
                            var metadata_2 = storage_1.defaultMetadataStorage.findTypeMetadata(targetType, propertyName);
                            if (metadata_2) {
                                var options = { newObject: newValue, object: value, property: propertyName };
                                var newType = metadata_2.typeFunction ? metadata_2.typeFunction(options) : metadata_2.reflectedType;
                                if (metadata_2.options && metadata_2.options.discriminator && metadata_2.options.discriminator.property && metadata_2.options.discriminator.subTypes) {
                                    if (!(value[valueKey] instanceof Array)) {
                                        if (this_2.transformationType === TransformationType.PLAIN_TO_CLASS) {
                                            type = metadata_2.options.discriminator.subTypes.find(function (subType) {
                                                if (subValue && metadata_2.options.discriminator.property in subValue) {
                                                    return subType.name === subValue[metadata_2.options.discriminator.property];
                                                }
                                            });
                                            type === undefined ? type = newType : type = type.value;
                                            if (!metadata_2.options.keepDiscriminatorProperty) {
                                                if (subValue && metadata_2.options.discriminator.property in subValue) {
                                                    delete subValue[metadata_2.options.discriminator.property];
                                                }
                                            }
                                        }
                                        if (this_2.transformationType === TransformationType.CLASS_TO_CLASS) {
                                            type = subValue.constructor;
                                        }
                                        if (this_2.transformationType === TransformationType.CLASS_TO_PLAIN) {
                                            subValue[metadata_2.options.discriminator.property] = metadata_2.options.discriminator.subTypes.find(function (subType) { return subType.value === subValue.constructor; }).name;
                                        }
                                    }
                                    else {
                                        type = metadata_2;
                                ...
                                isSubValueMap = isSubValueMap || metadata_2.reflectedType === Map;
                            }
                            else if (this_2.options.targetMaps) { // try to find a type in target maps
                                this_2.options.targetMaps
                                    .filter(function (map) { return map.target === targetType && !!map.properties[propertyName]; })
                                    .forEach(function (map) { return type = map.properties[propertyName]; });
                            }
                            else if (this_2.options.enableImplicitConversion && this_2.transformationType === TransformationType.PLAIN_TO_CLASS) {
                                // if we have no registererd type via the @Type() decorator then we check if we have any
                                // type declarations in reflect-metadata (type declaration is emited only if some decorator is added to the property.)
                                var reflectedType = Reflect.getMetadata("design:type", targetType.prototype, propertyName);
                                if (reflectedType) {
                        ...
                        // if value is an array try to get its custom array type
                        var arrayType_2 = Array.isArray(value[valueKey]) ? this_2.getReflectedType(targetType, propertyName) : undefined;
                        // const subValueKey = TransformationType === TransformationType.PLAIN_TO_CLASS && newKeyName ? newKeyName : key;
                        var subSource = source ? source[valueKey] : undefined;
                        // if its deserialization then type if required
                        // if we uncomment this types like string[] will not work
                        ...
                        if (newValue.constructor.prototype) {
                            var descriptor = Object.getOwnPropertyDescriptor(newValue.constructor.prototype, newValueKey);
                            if ((this_2.transformationType === TransformationType.PLAIN_TO_CLASS || this_2.transformationType === TransformationType.CLASS_TO_CLASS)
                                && ((descriptor && !descriptor.set) || newValue[newValueKey] instanceof Function)) //  || TransformationType === TransformationType.CLASS_TO_CLASS
                                return "continue";
                        }
                        if (!this_2.options.enableCircularCheck || !this_2.isCircular(subValue)) {
                            var transformKey = this_2.transformationType === TransformationType.PLAIN_TO_CLASS ? newValueKey : key;
                            var finalValue = void 0;
                            if (this_2.transformationType === TransformationType.CLASS_TO_PLAIN) {
                                // Get original value
                                finalValue = value[transformKey];
                        ...
                        else if (this_2.transformationType === TransformationType.CLASS_TO_CLASS) {
                            var finalValue = subValue;
                            finalValue = this_2.applyCustomTransformations(finalValue, targetType, key, value, this_2.transformationType);
                            if (newValue instanceof Map) {
                                newValue.set(newValueKey, finalValue);
                            }
                            else {
